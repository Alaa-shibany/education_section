...
class TeachersRepository {
  ...
  Future<Either<Failure, PaginationModel<teacherModel>>> getTeachersData({ required int page, required String search, required String phone, required String email,  }) async {
    try {
      final response = await _apiService.get(
        EndPoints.teachers, queryParams: {'page': page, 'search': search, 'phone': phone, 'email': email}
      );

      final items = (response.data['data']['data'] as List)
          .map((i) => teacherModel.fromJson(i)).toList();
